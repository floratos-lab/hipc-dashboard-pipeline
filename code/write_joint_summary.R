...
write_joint_summary <- function(response_types, exposure_type, log_files) {
  ...
  for(i in 1:length(response_types)) {
    test_filename <- logfile_path(log_files, response_types[i], "exposure_material_id.txt")
  ...
  summarize_response <- function(response_types, type, type_file, header, get_column, log_files) {

    joint_items <- lapply(response_types, function(x) {
      s <- read.table(file = logfile_path(log_files, x, type_file),
  ...
  write.csv(joint_summary, file = logfile_path(log_files, NULL,
